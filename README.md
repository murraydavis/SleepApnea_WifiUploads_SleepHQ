## WiFi Uploads of Sleep Apnea Data to the SleepHQ website and to the Oscar App

### Goal and Solution

My goal was to automate the process of getting the nightly sleep data from my ResMed AirSense11 to my laptop. Each morning, I removed a non-WiFi SD card from my AS11, took the SD card to my laptop, inserted the SD card into a USB hub, and then uploaded the data to both the Oscar app and to the SleepHQ website. I then had to eject the SD card and return it to the AS11. Rinse and repeat each morning. With apologies to the ghost of Lewis Carroll

```
Beware the Linuxwock, my son
The jaws that bite, the claws that catch!
Beware the Bashbash bird, and shun
The frumious LineCommanderSnatch!”
```

My LINUX solution automates this process by running a script on my Fedora 39 laptop. This script calls/invokes a remote script on a RaspberryPi box that copies the AS11 CPAP data from its WiFi SD card to a RaspberryPi box. This script next runs a ssh copy command to wirelessly transfer the CPAP data from the RaspberryPi box to my laptop. The Fedora 39 script is automatically run at 8:00 a.m. each morning via a crontab job. A no touch solution. The only manual process was to hit the Shift-F2 key combo in Oscar to upload the data to the app. I am still working on a process to automate the transfer of the CPAP data to the SleepHQ website. For now, the old drag and drop method.
...
10. The first device that connects to its WiFi gets the 1st IP address, 192.168.4.2. In my case, my iPhone was the 1st device to connect to its WiFi network, so it got that address. RPi was the second device to connect, so it got 192.168.4.3.
...
10. Now that you have the data on your home PC, copy the data to SleepHQ, step 16 or 17. Install the sshpass package to use with the ssh and scp commands, also in step 16.
...
1. Raspberry Pi Device (the model you purchased in Step 2), either a RaspberryPi 3,4, or 5.
...
The first thing that I always do with a new Linux system is update it.
...
On a RPi 3 system, you need to install the Network Manager application (sudo apt install network-manager) and enable it with raspi-config [by following these steps](https://pimylifeup.com/raspberry-pi-network-manager/#enabling-network-manager-on-the-raspberry-pi). Reboot.
...
John Boulden from the SleepHQ community began implementing my solution. He encountered a problem at this step using raspi-config on his RPi 4 system. It appears that Network Manager and the nmcli command are available as a default configuration. Issue the following command. If you get a version number on your RPi 4 or 5 box, Network Manager is installed and working. Proceed to step 9.1.
...
rpi@cpap:~ $ sudo -i  # Note, this command doe not work on a Fedora PC. You have to use: sudo su -, which works on both a RPi and Fedora PC.
...
# You are now logged back on as rpi in the last working directory.
...
# See Chapter 6 of my book, pages 147-167, for a quick overview of some of the basic vi commands. There is also an iPhone app called Vimmy which is a quick and dirty reference library.
...
sudo nmcli connection up yourwifihomenetworkSSID
...
1. line 15: sudo nmcli connection down mywifinetworkSSID, change mywifinetworkSSID
...
5. line 262: mv /home/rpi/cpapsrc/AirSense11-Data/STR.EDF /home/rpi/cpapsrc/AirSense11-Data/STR.edf -f, modify if you used a different username and datapath

6. line 266: sudo nmcli connection up mywifinetworkSSID, change mywifinetworkSSID
...
Are you curious what the directory on the ezShare SD card looks like? Open a web browser other than Firefox and type this URL: http://192.168.4.1/dir?dir=A:
...
Step 16 is just shown to illustrate the operation of the script. You will actually be using the method outlined in step  19. I prefer using the command line because it is faster than using VNC to connect to RPi. I find working at the RPi desktop via VNC to be slow because a base RPi PC simply does not have enough resources to create a speedy desktop environment. I want to do the drag and drop of the data files from my home PC instead of from RPi to the SleepHQ website.

On RPi, copy the data to home PC.
...
# There is a bit of a security risk in using this command because now the captain password shows up in the history command. History is the command to list previously used commands. A better alternative is to create a password file with only one line, the captain password.
...
The above is the method we would use if we were working from RPi. The final solution has us working from our home PC.

Now that you have last night's data on your home PC, open a file explorer application such as Nemo, drag and drop the files/folders to the SleepHQ site using the Data Import menu. An interesting note about the Data Import window. On the right-hand side, the files that are needed are listed: STR.edf, Identification.crc, Identification.json; as well as the folders: DATALOG and SETTINGS. These file and folder names are case sensitive. If SleepHQ detects that any of these items are missing, you will see the missing files highlighted in red.

#### Step 17  How to get ResMed data to SleepHQ. Option 2. Use VNC to connect to RPi and drag and drop from there.

As with Step 16, the preferred method is step 19. This info is just presented for illustration.
...
One can easily run commands on a remote system. We want to execute getdata_ezshare.sh every morning. Let's run that command from the home PC instead of from RPi. These commands are run on your home PC. Again, step 18 will provide some context as to why we choose to run all commands on our home PC as outlined in step 19.
...
The cursor at the command prompt disappears for a short while and then the output that would normally appear on RPi is displayed on your home PC, output similar to that from Step 15. You then can drag and drop the files from the AirSense11-Data folder to the SleepHQ site or upload the files to the Oscar app.

As an aside, you can execute any command, even builtin commands using this method.
...
### Step 19 The final 'no touch' solution.

Please read the cron_logrotate.md document. I explain in detail how to complete the tasks in step 18 by creating a simple script that automatically runs at 8:00 a.m. every morning. The script first copies the CPAP data from the ezShare WiFi card to RPi. The script next copies this CPAP data to my home computer. The document also explains how to create a logfile for recording the copy process and how to rotate this log on a weekly basis. The log was very useful to look at when I was troubleshooting the execution of my script.

### Future tasks, unfinished business.

1. Figure out an API or scripting solution to upload the CPAP data to the SleepHQ website. This would eliminate the drag and drop process. Early prospects are to use the Selenium package and the Chrome webdriver within a Python language script.
...
6. Find a solution for password less SSH keys bypassing the need for SSH passwords. Maybe using ssh-agent, etc.
